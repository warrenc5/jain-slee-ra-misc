        ...
        **/
        @Generated(value={})
        public class AccountBeforeRefillImpl implements AccountBeforeRefill,Codeable{
              static EventTypeID eventTypeID = new EventTypeID("ucip.AccountBeforeRefill","MOFOKOM","4.1");
...
UCIPResourceAdaptorUsageParameters usage = (UCIPResourceAdaptorUsageParameters)raUsageParameters; 
              if(ok) {
              usage.sampleAccountBeforeRefillLatencyTxOK(System.currentTimeMillis()-then);
              usage.incrementAccountBeforeRefillTxOK(1L);
              }else {
              usage.incrementAccountBeforeRefillTxNOK(1L);
              usage.sampleAccountBeforeRefillLatencyTxNOK(System.currentTimeMillis()-then);
              }
               }
            @Override
            public void encode(Encoder encoder) {
              encoder.start("AccountBeforeRefill", this);
        ...
        if (communityIdList!=null) 
            ...
            public void decode(Decoder decoder) throws InstantiationException, IllegalAccessException{
              decoder.start("AccountBeforeRefill", this);
