        ...
        **/
        @Generated(value={})
        public class RefillValueTotalImpl implements RefillValueTotal,Codeable{
              static EventTypeID eventTypeID = new EventTypeID("ucip.RefillValueTotal","MOFOKOM","4.1");
...
UCIPResourceAdaptorUsageParameters usage = (UCIPResourceAdaptorUsageParameters)raUsageParameters; 
              if(ok) {
              usage.sampleRefillValueTotalLatencyTxOK(System.currentTimeMillis()-then);
              usage.incrementRefillValueTotalTxOK(1L);
              }else {
              usage.incrementRefillValueTotalTxNOK(1L);
              usage.sampleRefillValueTotalLatencyTxNOK(System.currentTimeMillis()-then);
              }
               }
            @Override
            public void encode(Encoder encoder) {
              encoder.start("RefillValueTotal", this);
        ...
        if (usageAccumulatorInformation!=null) 
            ...
            public void decode(Decoder decoder) throws InstantiationException, IllegalAccessException{
              decoder.start("RefillValueTotal", this);
