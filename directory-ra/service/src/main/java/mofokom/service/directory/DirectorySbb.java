 ...
 */
package mofokom.service.directory;
...
import mofokom.in.directory.resource.DirectoryActivityContextInterfaceFactory;
import mofokom.in.directory.resource.DirectorySbbInterface;
import mofokom.service.directory.usage.DirectoryUsageParameters;

/**
 *
 * @author wozza
 */
public abstract class DirectorySbb implements Sbb {

    private DirectoryActivityContextInterfaceFactory directoryACIFactory;
    private DirectorySbbInterface directorySbbInterface;
    ...
    public void onServiceStarted(javax.slee.serviceactivity.ServiceStartedEvent event, ActivityContextInterface aci) {
        try {
            ...
            Logger.getLogger(DirectorySbb.class.getName()).log(Level.SEVERE, null, ex);
        } catch (Exception ex) {
            Logger.getLogger(DirectorySbb.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    public void setSbbContext(javax.slee.SbbContext context) {
        ...
        try {
            ...
            String directoryAciFactoryName = (String) env.lookup(BASIC_ACI_FACTORY_NAME);
            String directorySbbRAInterfaceName = (String) env.lookup(BASIC_SBB_RA_INTERFACE_NAME);
            directorySbbInterface = (DirectorySbbInterface) env.lookup(directorySbbRAInterfaceName.trim());
            directoryACIFactory = (DirectoryActivityContextInterfaceFactory) env.lookup(directoryAciFactoryName.trim());
        } catch (NamingException ex) {
    ...
    public abstract DirectoryUsageParameters getDefaultSbbUsageParameterSet();
