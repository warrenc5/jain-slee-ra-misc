...
final public class Rule$infoCode extends Rule
{
  private Rule$infoCode(String spelling, ArrayList<Rule> rules)
  {
  ...
  public static Rule$infoCode parse(ParserContext context)
  {
    context.push("infoCode");
    ...
    {
      {
        ...
        if (parsed)
        {
          ...
          for (int i1 = 0; i1 < 1 && f1; i1++)
          {
            rule = Terminal$StringValue.parse(context, "LC");
            if ((f1 = rule != null))
            {
              e1.add(rule);
              c1++;
    ...
    rule = null;
    if (parsed)
      rule = new Rule$infoCode(context.text.substring(s0, context.index), e0);
    else
      context.index = s0;

    context.pop("infoCode", parsed);

    return (Rule$infoCode)rule;
  }
}
