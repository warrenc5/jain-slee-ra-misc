...
public abstract class AssociationEvent extends EventObject {
     ...
     *
     * @param source A reference to the object, the "source", that is logically
     * deemed to be the object upon which the Event in question initially
     * occurred.
     * @param assocHandle The association handle to uniquely identify the MG-MGC
     * pair. This is allocated by the stack when the Listener registers with the
     * provider with a unique MG-MGC identity.
     * @throws IllegalArgumentException if the parameters send in the
     * constructor are invalid.
     */
    protected AssociationEvent(java.lang.Object source, int assocHandle)
     ...
     * association creation/ deletion/ modification request event or association
     * creation or deletion or modification response event or association
     * indication event.
     */
    ...
    public boolean equals(Object oo) {
        if (oo == null || (!this.getClass().isAssignableFrom(oo.getClass()) || !oo.getClass().getName().equals(this.getClass().getName())))
            return false;
        javax.megaco.AssociationEvent o = (javax.megaco.AssociationEvent) oo;
        if (assocHandle != o.assocHandle)
            return false;
