 ...
 */
package mofokom.service.telnet;
...
import mofokom.in.telnet.resource.TelNetActivityContextInterfaceFactory;
import mofokom.in.telnet.resource.TelNetResourceAdaptorInterface;
import mofokom.service.telnet.usage.TelNetUsageParameters;

/**
 *
 * @author wozza
 */
public abstract class TelNetSbb implements Sbb {

    private TelNetActivityContextInterfaceFactory telnetACIFactory;
    private TelNetResourceAdaptorInterface telnetSbbInterface;
    private static final String TELNET_ACI_FACTORY_NAME = "TELNET_ACI_FACTORY_NAME";
    private static final String TELNET_SBB_RA_INTERFACE_NAME = "TELNET_SBB_RA_INTERFACE_NAME";
    ...
    public void setSbbContext(javax.slee.SbbContext context) {
        ...
        try {
            ...
            String telnetAciFactoryName = (String) env.lookup(TELNET_ACI_FACTORY_NAME);
            String telnetSbbRAInterfaceName = (String) env.lookup(TELNET_SBB_RA_INTERFACE_NAME);
            telnetSbbInterface = (TelNetResourceAdaptorInterface) env.lookup(telnetSbbRAInterfaceName.trim());
            telnetACIFactory = (TelNetActivityContextInterfaceFactory) env.lookup(telnetAciFactoryName.trim());
        } catch (NamingException ex) {
    ...
    public abstract TelNetUsageParameters getDefaultSbbUsageParameterSet();
    ...
    public void onConnectionEvent(mofokom.in.telnet.event.ConnectionEvent event, ActivityContextInterface aci, EventContext context) {
        tracer.info("event ConnectionEvent");
    }

    public void onTelNetEvent(mofokom.in.telnet.event.TelNetEvent event, ActivityContextInterface aci, EventContext context) {
        tracer.info("event TelNetEvent");
    }
}
